...
safe. Input mid-turn on a Codex session may return a retryable invalid-state (turns are serialized).
...
(`pending`→`started`→`closed`/`crashed`/`failed`), `coding_agent_type`, and `model`. State is
