...
- `session-send-prompt` — `_ps`: `session_id` + `content` are the only fields validated locally
  (missing either FAILs the task); `runtime_name` is forwarded to the downstream input endpoint and
  required *there*, not checked here; `wait` (bool, DEFAULT true).
...
`session-status` → `{found, state}`; `session-get-last-message` → `{found, message, stop_reason}`;
